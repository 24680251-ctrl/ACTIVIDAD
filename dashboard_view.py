...
class DashboardView(ft.Container):
    ...
    def _build_ui(self):
        ...
        # --- Tarjetas KPI ---
        kpis = ft.Row([
            self._kpi_card("Ventas Hoy",  f"${float(data['ventas_hoy']):.2f}",  Icons.TRENDING_UP, "#4ade80"),
            self._kpi_card("Gastos Hoy",  f"${float(data['gastos_hoy']):.2f}",  Icons.TRENDING_DOWN, "#f87171"),
            self._kpi_card("Ganancia",    f"${float(data['ventas_hoy']) - float(data['gastos_hoy']):.2f}",  Icons.ACCOUNT_BALANCE_WALLET, "#38bdf8"),
        ...
        max_v = max((float(d["total"]) for d in historico), default=1) or 1
        chart_h = 140
        ...
        puntos = ft.Row(
            ...
            controls=[
                ft.Column([
                    ft.Text(f"${float(d['total']):.0f}", size=9, color="#38bdf8", text_align="center"),
                    ft.Container(
                        width=28,
                        height=max(4, int((float(d["total"]) / max_v) * chart_h)),
                        bgcolor="#38bdf8",
